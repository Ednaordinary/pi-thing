...
fn i_compute_pqt(n1: u64, n2: u64) -> PQT {
    ...
    unsafe {
        if n1 + 1 == n2 {
            ...
            println!(
                "m {} {} {n1} {n2}",
                make_cstr_mpz(res1.p),
                make_cstr_mpz(res2.p)
            );
            gmp::mpz_mul(
    ...
    unsafe {
        if n1 + 1 == n2 {
            ...
            if n2 - n1 < 10u64.pow(5) {
                ...
                println!("1 {}", make_cstr_mpz(res2.p));
                if n2 - n1 > 1000000 {
                    println!("{}", n2);
                }
                // p = res1 p * res2 p
...
async fn main() {
    ...
    unsafe {
        ...
        println!("pqt done");
        println!("{}", make_cstr_mpz(pqt.p));
        let mut pi = allocate_mpf(0, prec);
        let mut e = allocate_mpf(E, prec); // e
